<template>
  <div class="template-container" :style="{ width: windowWidth + 'px' }">
    <header>
      <h1>📝 SQL 脚本生成器</h1>
    ...
    <div class="main-content">
      ...
      <div class="action-buttons">
        ...
        <button class="btn btn-default" @click="openSettings">⚙️ 设置</button>
      </div>
...
async function generate() {
  ...
  if (!finalDirName) {
    ...
    lastAutoDirName.value = finalDirName
  }
  ...
  try {
    const res = await window.electronAPI.generateScript({
...
async function openFile() {
  if (result.value?.filePath) {
    await window.electronAPI.openFile(result.value.filePath)
  }
}

async function openFolder() {
  if (result.value?.targetPath) {
    await window.electronAPI.openFolder(result.value.targetPath)
  }
}
...
onMounted(async () => {
  ...
  scriptTypes.value = config.script_types || []
  updateDirName(true)

  window.electronAPI?.onConfigChanged(async () => {
    const newConfig = await window.electronAPI.getConfig()
    databases.value = newConfig.databases || []
    scriptTypes.value = newConfig.script_types || []
    updateDirName(true)
  })
})
...
.template-container header::before {
  content: '';
  position: absolute;
  top: 0;
  left: 0;
...
.template-container header .subtitle {
  position: relative;
  z-index: 1;
}
